...
export const updateProfilPicture = async (userId, urlProfil) => {
  try {
    if (!userId || !urlProfil) return null;
    const newProfilPicture = await prisma.users.update({
      where: { user_id: userId },
      data: {
        profile_picture: urlProfil,
      },
    });
    console.log(newProfilPicture);
    return newProfilPicture !== null ? newProfilPicture.profile_picture : null;
  } catch (error) {
    console.log(error);
    return null;
  } finally {
